    ...
    <form method="GET" action="{{ route('alertas.index') }}" class="row g-2 align-items-end">
        ...
        <div class="col-12 col-md-3">
            ...
            <select name="obra_id" class="form-select form-select-sm">
                ...
                <option value="{{ $obra->id }}" {{ request('obra_id') == $obra->id ? 'selected' : '' }}>{{ $obra->nombre }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-12 col-md-3">
...
<div class="table-card">
    <div class="table-header">
        <h6>
            <span class="material-icons" style="font-size:18px;vertical-align:middle;color:#E53935">notifications_active</span>
            Alertas ≥ 85 dB — Trabajadores
        </h6>
    ...
    <div class="table-responsive">
        <table class="table table-hover mb-0">
            ...
            @empty
            <tr><td colspan="8" class="text-center text-muted py-4">No hay alertas ≥ 85 dB registradas.</td></tr>
            @endforelse
            </tbody>
        </table>
    </div>
